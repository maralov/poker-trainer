...
const TIGHTER2={"UTG":"UTG","UTG+1":"UTG","MP":"UTG","LJ":"UTG+1","HJ":"MP","CO":"LJ","BTN":"HJ","SB":"CO"};

const VS_RAISE=(()=>{const T={
  ...
  LATE:{label:"BTN або SB (стіл-рейз)",raise:S("99+","AJs+","KQs","AQo+","A4s-A5s"),
    call:{POS:S("22-88","A7s-ATs","KJs","QJs","JTs","T9s","AJo","KQo"),SB:S("66-88","ATs"),
          BB:S("22-88","A2s-ATs","K7s+","Q8s+","J8s+","T8s+","97s+","86s+","75s+","65s","54s",
               "A8o-AJo","KTo+","QTo+","JTo","T9o")},
    note:"Найширший діапазон опенера за столом. З BB захищайся широко — ти вже вклав блайнд і закриваєш торги. З SB навпаки: майже все — 3-бет або фолд, колл лишається вузьким винятком."}
};
  for(const b of Object.keys(T)) for(const c of Object.keys(T[b].call))
    T[b].call[c]=minus(T[b].call[c],T[b].raise);
  return T;})();
...
function preBuildSpot(force){
  ...
  if(scen==="rfi"){
    ...
    const hi=force?Math.min(7,Math.max(1,ACTION_ORDER.indexOf(force.heroPos))):1+Math.floor(Math.random()*7);
